...
import { useListTableColumn } from "./column";
...
type APIResponse = RouterOutputs['extension']['getLists']
export type Rows = APIResponse[number]
type ListsTableProps = {

}

export function ListsTable(props: ListsTableProps) {
  const columns = useListTableColumn()
  const trpc = useTRPC();
  const [query, setQuery] = useState('')
  const [pagination, setPagination] = useState({ page: 1, limit: 50 })
  const { data, isLoading } = useQuery(
    trpc.extension.getLists.queryOptions()
  )
