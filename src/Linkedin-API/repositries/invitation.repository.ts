...
import {
  INVITATION_TYPE,
  LinkedInInvitation,
} from "../entities/linkedin-invitation.entity";
...
const parseInvitationResponse =
  <T extends GetSentInvitationResponse | GetReceivedInvitationResponse>(
    ...
    (response: T): Invitation[] => {
      ...
      const profiles = keyBy(
        () => { }
        // getProfilesFromResponse<T>(response), 'entityUrn'
      );
      const invitations = results.filter(
        (r) => r.$type === INVITATION_TYPE && !!r[idField]
...
const parseSentInvitations =
  parseInvitationResponse<GetSentInvitationResponse>(TO_MEMBER_FIELD);
const parseReceivedInvitations =
  parseInvitationResponse<GetReceivedInvitationResponse>(FROM_MEMBER_FIELD);
...
export class InvitationRepository {
  ...
  async sendInvitation({
    ...
    await this.client.request.invitation.sendInvitation({
      profileId,
      message,
    });
  ...
  private async fetchReceived({ skip = 0, limit = 100 } = {}): Promise<
    Invitation[]
  > {
    const response = await this.client.invitation.getReceivedInvitations({
      skip,
      limit,
    });
  ...
  private async fetchSent({ skip = 0, limit = 100 } = {}): Promise<
    Invitation[]
  > {
    const response = await this.client.invitation.getSentInvitations({
      skip,
      limit,
    });
